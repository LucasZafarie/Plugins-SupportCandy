...
if ( ! class_exists( 'WPSC_RP_Radio_Button' ) ) :

	final class WPSC_RP_Radio_Button {
		...
		public static function init() {
			...
			add_action( 'wp_ajax_wpsc_rp_get_radio_select', array( __CLASS__, 'layout' ) );
			add_action( 'wp_ajax_wpsc_rp_run_cfrs_report', array( __CLASS__, 'run_cfrs_reports' ) );
		}
		...
		public static function add_section( $sections ) {
			...
			foreach ( $settings['cf-reports'] as $id ) {
				...
				if ( $cf->id && class_exists( $cf->type ) && $cf->type::$slug == 'cf_radio_button' ) {
					$sections[ $cf->slug ] = array(
						'slug'     => $cf->slug,
						'icon'     => 'chart-bar',
						'label'    => $cf->name,
						'callback' => 'wpsc_rp_get_cf_radio_select',
						'run'      => array(
							'shortname' => 'cfrs',
							'function'  => 'wpsc_rp_run_cfrs_report',
						),
					);
				}
			}
		...
		/**
		 * Print radio select type custom field report layout
		 *
		 * @return void
		 */
		...
		/**
		 * Run radio select report
		 *
		 * @return void
		 */
		public static function run_cfrs_reports() {
