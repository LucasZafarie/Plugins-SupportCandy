...
if ( ! class_exists( 'WPSC_RP_Checkbox' ) ) :

	final class WPSC_RP_Checkbox {
		...
		public static function init() {
			...
			add_action( 'wp_ajax_wpsc_rp_get_checkbox', array( __CLASS__, 'layout' ) );
			add_action( 'wp_ajax_wpsc_rp_run_cfcheck_report', array( __CLASS__, 'run_cfcheck_reports' ) );
		}
		...
		public static function add_section( $sections ) {
			...
			foreach ( $settings['cf-reports'] as $id ) {
				...
				if ( $cf->id && class_exists( $cf->type ) && $cf->type::$slug == 'cf_checkbox' ) {
					$sections[ $cf->slug ] = array(
						'slug'     => $cf->slug,
						'icon'     => 'chart-bar',
						'label'    => $cf->name,
						'callback' => 'wpsc_rp_get_cf_checkbox',
						'run'      => array(
							'shortname' => 'cfcheck',
							'function'  => 'wpsc_rp_run_cfcheck_report',
						),
					);
				}
			}
		...
		/**
		 * Print checkbox type custom field report layout
		 *
		 * @return void
		 */
		 ...
		 * @return void
		 */
		public static function run_cfcheck_reports() {
