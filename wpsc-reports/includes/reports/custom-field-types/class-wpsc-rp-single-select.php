...
if ( ! class_exists( 'WPSC_RP_Single_Select' ) ) :

	final class WPSC_RP_Single_Select {
		...
		public static function init() {
			...
			add_action( 'wp_ajax_wpsc_rp_get_single_select', array( __CLASS__, 'layout' ) );
			add_action( 'wp_ajax_wpsc_rp_run_cfss_report', array( __CLASS__, 'run_cfss_reports' ) );
		}
		...
		public static function add_section( $sections ) {
			...
			foreach ( $settings['cf-reports'] as $id ) {
				...
				if ( $cf->id && class_exists( $cf->type ) && $cf->type::$slug == 'cf_single_select' ) {
					$sections[ $cf->slug ] = array(
						'slug'     => $cf->slug,
						'icon'     => 'chart-bar',
						'label'    => $cf->name,
						'callback' => 'wpsc_rp_get_cf_single_select',
						'run'      => array(
							'shortname' => 'cfss',
							'function'  => 'wpsc_rp_run_cfss_report',
						),
					);
				}
			}
		...
		public static function layout() {
			...
			if ( check_ajax_referer( $cf_slug, '_ajax_nonce', false ) !== 1 ) {
				wp_send_json_error( 'Unauthorised request!1', 400 );
			}
		...
		/**
		 * Run single select report
		 *
		 * @return void
		 */
		public static function run_cfss_reports() {
