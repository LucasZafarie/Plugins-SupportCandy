...
if ( ! class_exists( 'WPSC_RP_Response_Delay' ) ) :

	final class WPSC_RP_Response_Delay {
		...
		public static function init() {
			...
			add_action( 'wp_ajax_wpsc_rp_get_response_delay', array( __CLASS__, 'layout' ) );
			add_action( 'wp_ajax_wpsc_rp_run_rd_report', array( __CLASS__, 'run_rd_reports' ) );
		}

		/**
		 * Print response delay report layout
		 *
		 * @return void
		 */
		public static function layout() {
			...
			<div class="wpsc-setting-header">
				<h2><?php esc_attr_e( 'Atraso da resposta', 'wpsc-reports' ); ?></h2>
			</div>
			...
			<?php WPSC_RP_Filters::layout( 'response_delay' ); ?>
			<div class="wpsc-setting-section-body">
				...
				<table class="wpsc-rp-tbl">
					<tr>
						<th><?php esc_attr_e( 'Atraso na Primeira Resposta (Horas)', 'wpsc-reports' ); ?></th>
						<th><?php esc_attr_e( 'Atraso Médio das Respostas (Horas)', 'wpsc-reports' ); ?></th>
					</tr>
					<tr>
						<td class="frd"></td>
						<td class="ard"></td>
		 ...
		 * @return void
		 */
		public static function run_rd_reports() {

			if ( check_ajax_referer( 'response_delay', '_ajax_nonce', false ) !== 1 ) {
			...
			// calculate first response delay and average response delay.
			$fr_delay       = 0;
			$total_fr_delay = 0;
			$ar_delay       = 0;
			$total_ar_delay = 0;
			...
			foreach ( $results as $ticket ) {
				...
				$fr_delay += intval( $ticket->frd );
				$ar_delay += intval( $ticket->ard );
				++$count;
			}

			// total response delays (hrs).
			if ( $fr_delay ) {

				$temp_int       = (int) ( $fr_delay / 60 );
				$temp_decimal   = $fr_delay % 60;
				$total_fr_delay = $temp_int + ( $temp_decimal / 60 );

				$temp_int       = (int) ( $ar_delay / 60 );
				$temp_decimal   = $ar_delay % 60;
				$total_ar_delay = $temp_int + ( $temp_decimal / 60 );
			}

			// average for current iteration (hrs).
			if ( $count ) {

				$fr_delay     = ceil( $fr_delay / $count );
				$temp_int     = (int) ( $fr_delay / 60 );
				$temp_decimal = $fr_delay % 60;
				$fr_delay     = $temp_int + ( $temp_decimal / 60 );

				$ar_delay     = ceil( $ar_delay / $count );
				$temp_int     = (int) ( $ar_delay / 60 );
				$temp_decimal = $ar_delay % 60;
				$ar_delay     = $temp_int + ( $temp_decimal / 60 );
			}

			$response['frd']          = round( $fr_delay, 2 );
			$response['totalFrDelay'] = round( $total_fr_delay, 2 );
			$response['ard']          = round( $ar_delay, 2 );
			$response['totalArDelay'] = round( $total_ar_delay, 2 );
