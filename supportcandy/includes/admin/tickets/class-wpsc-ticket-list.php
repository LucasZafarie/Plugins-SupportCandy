...
if ( ! class_exists( 'WPSC_Ticket_List' ) ) :
	...
	final class WPSC_Ticket_List {
			...
			<div class="wpsc-filter">
				<div class="wpsc-search">
					<div class="search-field">
						...
						<input class="wpsc-search-input" type="text" placeholder="<?php esc_attr_e( 'Buscar...', 'supportcandy' ); ?>" spellcheck="false" value="<?php echo esc_attr( stripslashes( self::$filters['search'] ) ); ?>" onkeyup="wpsc_tl_search_keyup(event, this);"/>
					</div>
				</div>
				<div class="wpsc-filter-container">
					<div class="wpsc-filter-item">
						<label for="wpsc-input-filter"><?php esc_attr_e( 'Filtros', 'supportcandy' ); ?></label>
						<select id="wpsc-input-filter" class="wpsc-input-filter" name="filter" onchange="wpsc_tl_filter_change(this);">

							<optgroup label="<?php esc_attr_e( 'Filtros padrões', 'supportcandy' ); ?>">
								<?php
					...
					<div class="wpsc-filter-item">
						<label for="wpsc-input-sort-by"><?php esc_attr_e( 'Ordenar por', 'supportcandy' ); ?></label>
					...
					<div class="wpsc-filter-submit">
						<button class="wpsc-button normal primary margin-right" onclick="wpsc_tl_apply_filter_btn_click();"><?php esc_attr_e( 'Aplicar', 'supportcandy' ); ?></button>
		...
		private static function get_filter_actions() {
			...
			$actions = array(
				'reset' => array(
					'label'    => esc_attr__( 'Redefinir', 'supportcandy' ),
				...
				<div class="wpsc-bulk-actions">
					...
					if ( $current_user->is_agent && self::$bulk_actions ) :
						...
						<button 
							...
							<?php esc_attr_e( 'Ações', 'supportcandy' ); ?>
					...
					<button 
						...
						<?php esc_attr_e( 'Lista de ações', 'supportcandy' ); ?>
						<?php WPSC_Icons::get( 'chevron-down' ); ?>
					</button>
					<div id="wpsc-more-actions" class="gpopover wpsc-popover-menu">
						...
						$more_actions = apply_filters(
							...
							array(
								'refresh'      => array(
									'icon'     => 'sync',
									'label'    => esc_attr__( 'Atualizar', 'supportcandy' ),
									'callback' => 'wpsc_get_tickets',
								),
								'auto-refresh' => array(
									'icon'     => 'history',
									'label'    => esc_attr__( 'Auto-Atualizar', 'supportcandy' ),
		...
		private static function get_pagination_html() {
			...
			<span 
				...
				<?php esc_attr_e( 'Próxima página', 'supportcandy' ); ?>
			</span>
			<?php
			if ( is_rtl() ) {
				?>
			...
			<span 
				...
				<?php esc_attr_e( 'Última página', 'supportcandy' ); ?>
			</span>
			<?php
		...
		private static function get_pagination_str() {
			...
			if ( self::$total_items == 1 ) {
				return esc_attr__( '1 Ocorrência', 'supportcandy' );
			}

			if ( self::$total_items <= self::$more_settings['number-of-tickets'] ) {
				/* translators: %1$s: total tickets */
				return sprintf( esc_attr__( '%1$d Ocorrências', 'supportcandy' ), self::$total_items );
			}
			...
			return sprintf( esc_attr__( '%1$d-%2$d de %3$d Ocorrências', 'supportcandy' ), $from, $to, self::$total_items );
		}
		...
		public static function get_tl_custom_filter_ui() {
			...
			<form action="#" onsubmit="return false;" class="wpsc-tl-custom-filter">
				...
				<div class="wpsc-input-group">
					<div class="label-container">
						<label for="">
							<?php esc_attr_e( 'Ordenar por', 'supportcandy' ); ?>
							<span class="required-char">*</span>
						</label>
					</div>
					<select name="sort-by">
			...
			<button class="wpsc-button small primary" onclick="wpsc_tl_apply_custom_filter(this);">
				<?php esc_attr_e( 'Aplicar', 'supportcandy' ); ?>
			</button>
			<?php
		...
		public static function bulk_assign_agents() {
			...
			<form action="#" onsubmit="return false;" class="frm-bulk-assign-agent">
				<div class="wpsc-input-group" style="flex-direction:row; flex-wrap:nowrap;">
					...
					<div class="wpsc-input-group">
						<div class="label-container">
							<label for=""><?php esc_attr_e( 'Ordenar por', 'supportcandy' ); ?></label>
						</div>
			...
			if ( ! $is_deleted ) {
				...
				if ( $current_user->is_agent && self::has_ticket_cap( 'cs' ) ) {
					$bulk_actions['change-status'] = array(
						'icon'     => 'gps-navigation',
						'label'    => esc_attr__( 'Alterar Status', 'supportcandy' ),
				...
				if ( $current_user->is_agent && self::has_ticket_cap( 'aa' ) ) {
					$bulk_actions['assign-agents'] = array(
						'icon'     => 'headset',
						'label'    => esc_attr__( 'Atribuir agentes', 'supportcandy' ),
				...
				if ( $current_user->is_agent && self::has_ticket_cap( 'tt' ) ) {
					$bulk_actions['assign-tags'] = array(
						'icon'     => 'tags',
						'label'    => esc_attr__( 'Atribuir Tags', 'supportcandy' ),
