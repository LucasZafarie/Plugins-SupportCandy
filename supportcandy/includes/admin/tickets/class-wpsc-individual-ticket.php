...
if ( ! class_exists( 'WPSC_Individual_Ticket' ) ) :
	...
	final class WPSC_Individual_Ticket {
		...
		public static function layout() {
			...
			<script>
				var up_img = '<?php WPSC_Icons::get( 'chevron-up' ); ?>';
				var down_img = '<?php WPSC_Icons::get( 'chevron-down' ); ?>';
				jQuery(document).ready(function() {
					...
					widgets_hidden.forEach(function(widget){
						...
						jQuery('.' + widget).find(".wpsc-widget-header .wpsc-itw-toggle").html(down_img);
					});
					jQuery('.wpsc-itw-toggle').on('click',function() {
						...
						if(widgets_hidden.includes(widget)){
							...
							jQuery('.' + widget).find(".wpsc-widget-header .wpsc-itw-toggle").html(up_img);
		...
		public static function load_actions() {
			...
			$actions      = array(
				'refresh' => array(
					'label'    => esc_attr__( 'Recarregar', 'supportcandy' ),
			...
			if ( self::$ticket->is_active ) {
				...
				if (
					...
					$actions['close'] = array(
						'label'    => esc_attr__( 'Fechar', 'supportcandy' ),
				...
				if ( self::$view_profile == 'agent' && self::has_ticket_cap( 'dt' ) ) {
					$actions['duplicate'] = array(
						'label'    => esc_attr__( 'Duplicar', 'supportcandy' ),
				...
				if ( $current_user->is_agent && self::has_ticket_cap( 'dtt' ) ) {
					$actions['delete'] = array(
						'label'    => esc_attr__( 'Apagar', 'supportcandy' ),
				...
				// Copy url.
				$actions['copy'] = array(
					'label'    => esc_attr__( 'Copiar link', 'supportcandy' ),
			...
			if ( self::$view_profile == 'agent' ) {
				...
				if ( self::has_ticket_cap( 'reply' ) || self::is_customer() ) {
					$submit_actions['reply'] = array(
						'icon'     => 'reply',
						'label'    => esc_attr__( 'Resposta', 'supportcandy' ),
				...
				if ( self::has_ticket_cap( 'pn' ) ) {
					$submit_actions['private-note'] = array(
						'icon'     => 'notes',
						'label'    => esc_attr__( 'Anotação privada', 'supportcandy' ),
		...
		public static function get_mobile_widgets() {
			...
			<div class="wpsc-it-body-item wpsc-it-mobile-widget-container wpsc-visible-xs wpsc-visible-sm">
				<div class="wpsc-it-mob-widget-trigger-btn" onclick="wpsc_toggle_mob_it_widgets();">
					<h2><?php esc_attr_e( 'Detalhes da ocorrência', 'supportcandy' ); ?></h2>
		...
		public static function get_reply_section() {
			...
			if ( self::$url_auth && ! $current_user->is_customer ) {
				...
				<div class="wpsc_url_auth_sign_in wpsc-widget-body">
					<p class="wpsc_auth_header"><?php esc_attr_e( 'Você deve entrar para responder', 'supportcandy' ); ?></p>
					<p class="wpsc_auth_link wpsc-signin-customer">
						<?php
							printf(
								/* translators: %s: Sign in */
								esc_attr__( '%s usando o email e senha', 'supportcandy' ),
								'<a class="wpsc-link" href="javascript:wpsc_user_sign_in();">' . esc_attr__( 'Entre', 'supportcandy' ) . '</a>'
							);
						?>
					</p>
					<?php
						...
						<div class="actions">
							<div class="wpsc-editor-actions">
								...
								if ( WPSC_Text_Editor::is_allow_attachments() ) :
									?>
									<span class="wpsc-link wpsc-it-attach" onclick="wpsc_trigger_desc_attachments('<?php echo esc_attr( $unique_id ); ?>');"><?php esc_attr_e( 'Anexos', 'supportcandy' ); ?></span>
									<?php
								endif;
								if ( $current_user->is_agent ) :
									?>
									<span class="wpsc-link wpsc-it-macro" onclick="wpsc_get_macros()"><?php esc_attr_e( 'Inserir Macro', 'supportcandy' ); ?></span>
									<?php
								endif;
						...
						<div class="submit-container">
							...
							if ( self::$view_profile == 'agent' ) {
								...
								<button 
									...
									<?php esc_attr_e( 'Enviar como', 'supportcandy' ); ?>
								</button>
								...
								<button 
									...
									<?php esc_attr_e( 'Enviar', 'supportcandy' ); ?>
								</button>
								<?php
								if ( (
									...
									<small class="thread-type">
										<i>
											...
											switch ( $thread->type ) {
												...
												case 'report':
													esc_attr_e( 'Reportar', 'supportcandy' );
													break;

												case 'reply':
													esc_attr_e( 'Resposta', 'supportcandy' );
													break;

												case 'note':
													esc_attr_e( 'Anotação', 'supportcandy' );
													break;
											}
											?>
										</i>
								...
								<div>
									...
									if ( $thread->customer ) {
										...
										printf(
											...
											esc_attr__( 'O %1$s foi altertado', 'supportcandy' ),
											'<strong>' . esc_attr( $cf->name ) . '</strong>'
										);
									}
									?>
		...
		public static function it_thread_info() {
			...
			$advanced   = get_option( 'wpsc-ms-advanced-settings', array() );
			$ip_address = $thread->ip_address ? $thread->ip_address : esc_attr__( 'Não aplicado', 'supportcandy' );
			$os         = $thread->os ? $thread->os : esc_attr__( 'Não aplicado', 'supportcandy' );
			$browser    = $thread->browser ? $thread->browser : esc_attr__( 'Não aplicado', 'supportcandy' );
			$source     = $thread->source && isset( WPSC_DF_Source::$sources[ $thread->source ] ) ? WPSC_DF_Source::$sources[ $thread->source ] : esc_attr__( 'Não aplicado', 'supportcandy' );
			...
			<div class="wpsc-edit-thread">
				<form onsubmit="return false;" class="edit-thread">
					...
					<div class="wpsc-it-editor-action-container">
						...
						<div class="submit-container">
							<button class="wpsc-button small primary margin-right" onclick="wpsc_it_set_edit_thread(this, '<?php echo esc_attr( $unique_id ); ?>')"><?php esc_attr_e( 'Enviar', 'supportcandy' ); ?></button>
