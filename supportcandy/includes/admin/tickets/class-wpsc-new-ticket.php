...
if ( ! class_exists( 'WPSC_New_Ticket' ) ) :
	...
	final class WPSC_New_Ticket {
		...
		public static function get_ticket_form() {
			...
			if ( $current_user->is_agent && in_array( $current_user->agent->role, $gs['allow-create-ticket'] ) && $current_user->agent->has_cap( 'create-as' ) ) :
				...
				<div style="width: 100%;">
					<div class="wpsc-tff customer wpsc-xs-12 wpsc-sm-12 wpsc-md-6 wpsc-lg-4">
						...
						<span class="extra-info">Selecione cliente para quem deseja criar uma ocorrência</span>
						...
						<div style="display:flex; flex-direction:column; width:100%;">
							...
							<a class="wpsc-link" href="javascript:wpsc_get_change_create_as('<?php echo esc_attr( wp_create_nonce( 'wpsc_get_change_create_as' ) ); ?>');" style="margin-top:3px; width:fit-content;"><?php esc_attr_e(' Adicionar novo', 'supportcandy' ); ?></a>
						</div>
						<script>
			...
			<div class="wpsc-tff wpsc-xs-12 wpsc-sm-12 wpsc-md-12 wpsc-lg-12">
				<div class="submit-container wpsc-ct-actions">
					<button id="wpsc-ct-submit" class="wpsc-button normal primary margin-right" onclick="wpsc_submit_ticket_form(this);"><?php esc_attr_e( 'Enviar', 'supportcandy' ); ?></button>
					<button id="wpsc-ct-reset" class="wpsc-button normal secondary" onclick="wpsc_get_ticket_form();"><?php esc_attr_e( 'Redefinir', 'supportcandy' ); ?></button>
		...
		public static function get_change_create_as() {
			...
			$title = esc_attr__(' Adicionar novo', 'supportcandy' );
