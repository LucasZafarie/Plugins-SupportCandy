			...
			<table class="wpsc_customer_list wpsc-setting-tbl">
				<thead>
					<tr>
						...
						<th><?php esc_attr_e( 'Número de ocorrência', 'supportcandy' ); ?></th>
							...
							<div>
								...
								if ( $log->modified_by ) {
									...
									printf(
										...
										esc_attr__( 'O %1$s foi alterado', 'supportcandy' ),
										'<strong>' . esc_attr( $cf->name ) . '</strong>'
									);
								}
						...
						<span>
							<div class="info-list-item">
								<div class="info-label">
									<a href="<?php echo esc_url_raw( admin_url( 'admin.php?page=wpsc-recent-activities&cid=' . $customer->id ) ); ?>" target="__blank"><?php echo esc_attr__( 'Ver todos', 'supportcandy' ); ?></a>
								</div>
							</div>
						</span>
					</div>
			...
			<div class="wpsc-widget-list-section">
				...
				if ( $logs['total_items'] ) {
					foreach ( $logs['results'] as $log ) {
						...
						if ( $log->type == 'report' ) {
							?>
						<div class="wpsc-widget-list"><?php echo esc_attr( $log->customer->name ) . ' ' . esc_attr__( 'registrou uma ocorrência', 'supportcandy' ); ?> <a href="<?php echo esc_attr( $url ); ?>" target="_blank">#<?php echo esc_attr( $log->ticket->id ) . ' ' . esc_attr( $log->ticket->subject ); ?></a><?php echo esc_attr( ' (' . $time_ago . ')' ); ?></div>
							<?php
