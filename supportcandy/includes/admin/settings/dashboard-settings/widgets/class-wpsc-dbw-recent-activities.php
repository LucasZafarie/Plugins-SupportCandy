				...
				<div class="wpsc-dash-widget-header">
					...
					if ( $current_user->agent->has_cap( 'backend-access' ) ) {
						...
						<div class="wpsc-dash-widget-actions">
							<a href="<?php echo esc_url_raw( admin_url( 'admin.php?page=wpsc-recent-activities' ) ); ?>" target="__blank"><?php echo esc_attr__( 'Ver todos', 'supportcandy' ); ?></a>
						</div>
						<?php
					}
					?>
			...
			<div class="wpsc-widget-list-section">
				...
				if ( $logs['total_items'] ) {
					foreach ( $logs['results'] as $log ) {
						...
						if ( $log->type == 'report' ) {
							?>
							<div class="wpsc-widget-list"><?php echo esc_attr( $log->customer->name ) . ' ' . esc_attr__( 'registrou uma ocorrência', 'supportcandy' ); ?>
