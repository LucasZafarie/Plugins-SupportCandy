	...
	final class WPSC_RA_Logs {
							...
							<div class="wpsc-ra-filter-container">
								<div class="wpsc-filter-container">
									...
									<div class="wpsc-filter-submit">
										<button type="button" class="wpsc-button normal primary" onclick="wpsc_get_activity_logs()"><?php esc_attr_e( 'Aplicar', 'supportcandy' ); ?></button>
										<div class="wpsc-filter-actions" style="margin-left: 5px;">
											<span class="wpsc-link">
												<a href="<?php echo esc_url( admin_url( 'admin.php?page=wpsc-recent-activities' ) ); ?>"><?php echo esc_attr__( 'Redefinir', 'supportcandy' ); ?></a>
											</span>
										</div>
									</div>
								</div>
		...
		public static function get_recent_activities() {
			...
			foreach ( $logs['results'] as $log ) {
				...
				if ( $log->type == 'report' ) {
					$log_str = '<span class="wpsc-ra-log-desc">' . $log->customer->name . ' registrou uma ocorrência <a href="' . admin_url( 'admin.php?page=wpsc-tickets&section=ticket-list&id=' . esc_attr( $log->ticket->id ) ) . '">#' . $log->ticket->id . ' ' . esc_attr( $log->ticket->subject ) . '</a></span>
