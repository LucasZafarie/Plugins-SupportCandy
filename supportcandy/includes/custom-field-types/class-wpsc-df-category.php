...
if ( ! class_exists( 'WPSC_DF_Category' ) ) :
	...
	final class WPSC_DF_Category {
		...
		public static function print_val( $cf, $val ) {
			...
			echo $category->id ? esc_attr( $category->name ) : esc_attr__( 'Não definido', 'supportcandy' );
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			$category = is_object( $val ) ? $val : new WPSC_Category( $val );
			return $category->id ? esc_attr( $category->name ) : esc_attr__( 'Não definido', 'supportcandy' );
		}
	}
endif;
