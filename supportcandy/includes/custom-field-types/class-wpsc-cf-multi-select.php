...
if ( ! class_exists( 'WPSC_CF_Multi_Select' ) ) :
	...
	final class WPSC_CF_Multi_Select {
		...
		public static function get_add_new_custom_field_properties( $field_class ) {
			...
			<div data-type="select-options" data-required="true" class="wpsc-input-group options">
				...
				<button class="wpsc-button small secondary" onclick="wpsc_get_add_new_option();">
					<?php esc_attr_e(' Adicionar novo', 'supportcandy' ); ?>
				</button>
			</div>
		...
		public static function print_val( $cf, $val ) {
			...
			echo $option_names ? esc_attr( implode( ', ', $option_names ) ) : esc_attr__( 'Não definido', 'supportcandy' );
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			$option_names = array_map( fn( $option ) => $option->name, $options );
			return $option_names ? esc_attr( implode( ', ', $option_names ) ) : esc_attr__( 'Não definido', 'supportcandy' );
		}
	}
endif;
