...
if ( ! class_exists( 'WPSC_CF_Number' ) ) :
	...
	final class WPSC_CF_Number {
		...
		public static function print_val( $cf, $val ) {
			...
			if ( $cf->number_type == 'float' ) {
				echo is_numeric( $val ) ? esc_attr( $val ) : esc_attr__( 'Não definido', 'supportcandy' );
			} else {
				echo is_numeric( $val ) ? intval( $val ) : esc_attr__( 'Não definido', 'supportcandy' );
			}
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			if ( $cf->number_type == 'float' ) {
				return is_numeric( $val ) ? esc_attr( $val ) : esc_attr__( 'Não definido', 'supportcandy' );
			} else {
				return is_numeric( $val ) ? intval( $val ) : esc_attr__( 'Não definido', 'supportcandy' );
			}
		}
	}
endif;
