...
if ( ! class_exists( 'WPSC_DF_Assigned_Agent' ) ) :
	...
	final class WPSC_DF_Assigned_Agent {
					...
					if ( $is_multiple && isset( $filter['operand_val_1'] ) ) {
						...
						foreach ( $filter['operand_val_1'] as $agent_id ) {
							...
							if ( $agent_id == '0' ) {
								...
								?>
								<option selected="selected" value="0"><?php esc_attr_e( 'Não definido', 'supportcandy' ); ?></option>
								<?php
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			$value = $agent_names ? implode( ', ', $agent_names ) : esc_attr__( 'Não definido', 'supportcandy' );
		...
		public static function print_widget_ticket_field_val( $cf, $ticket ) {
			...
			if ( $agents ) {
				...
				if ( $ms_advanced['self-assign'] && $ticket->is_active && WPSC_Individual_Ticket::$view_profile == 'agent' && $current_user->agent->has_cap( 'aa-unassigned' ) ) {
					?>
					<a class="wpsc-widget-default wpsc-link" onclick="wpsc_self_assign_ticket(<?php echo esc_attr( $ticket->id ); ?>, '<?php echo esc_attr( wp_create_nonce( 'wpsc_self_assign_ticket' ) ); ?>')"><?php esc_attr_e( 'Atribuir para mim', 'supportcandy' ); ?></a>
					<?php
				} else {
					?>
					<div class="wpsc-widget-default"><?php esc_attr_e( 'Não definido', 'supportcandy' ); ?></div>
					<?php
				}
				?>
				<?php
		...
		public static function print_val( $cf, $val ) {
			...
			echo $agent_names ? esc_attr( implode( ', ', $agent_names ) ) : esc_attr__( 'Não definido', 'supportcandy' );
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			return $agent_names ? esc_attr( implode( ', ', $agent_names ) ) : esc_attr__( 'Não definido', 'supportcandy' );
		}
	}
endif;
