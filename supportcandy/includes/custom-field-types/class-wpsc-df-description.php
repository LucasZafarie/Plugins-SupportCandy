	...
	final class WPSC_DF_Description {
		...
		public static function print_tff( $cf, $tff ) {
			...
			<div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<span class="extra-info">Insira uma descrição para essa ocorrência</span>
					...
					<div class="actions">
						<div class="wpsc-editor-actions">
							...
							if ( WPSC_Text_Editor::is_allow_attachments() ) {
								?>
								<span class="wpsc-link wpsc-ct-attach" onclick="wpsc_trigger_desc_attachments('<?php echo esc_attr( $unique_id ); ?>');"><?php esc_attr_e( 'Anexos', 'supportcandy' ); ?></span>
								<?php
							}

							if ( $current_user->is_agent ) {
								?>
								<span class="wpsc-link wpsc-ct-macro" onclick="wpsc_get_macros()"><?php esc_attr_e( 'Inserir Macro', 'supportcandy' ); ?></span>
								<?php
							}
