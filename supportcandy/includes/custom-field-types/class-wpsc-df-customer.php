...
if ( ! class_exists( 'WPSC_DF_Customer' ) ) :
	...
	final class WPSC_DF_Customer {
		...
		public static function print_val( $cf, $val ) {
			...
			echo $customer->id ? esc_attr( $customer->name ) : esc_attr__( 'Não definido', 'supportcandy' );
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			$customer = is_object( $val ) ? $val : new WPSC_Customer( $val );
			return $customer->id ? esc_attr( $customer->name ) : esc_attr__( 'Não definido', 'supportcandy' );
		}
	}
endif;
