...
if ( ! class_exists( 'WPSC_Installation' ) ) :
	...
	final class WPSC_Installation {
		...
		public static function initial_setup() {
			...
			$extra_info = esc_attr__( 'Select customer for whom you wish to create a ocorrência', 'supportcandy' );
			$wpdb->insert(
				$wpdb->prefix . 'psmsc_custom_fields',
				array(
					'name'       => $name,
			...
			$name = esc_attr__( 'Assunto', 'supportcandy' );
			$extra_info = "Breve descrição da ocorrência";
			$wpdb->insert(
				$wpdb->prefix . 'psmsc_custom_fields',
				array(
					'name'          => $name,
			...
			$extra_info = esc_attr__( 'Detalhes da ocorrência', 'supportcandy' );
			$wpdb->insert(
				$wpdb->prefix . 'psmsc_custom_fields',
				array(
					'name'          => $name,
					'extra_info'    => "Detalhes da ocorrência",
					'slug'          => 'description',
					'field'         => 'ticket',
					'type'          => 'df_description',
					'default_value' => 'Não aplicado',
			...
			$name = esc_attr__( 'Atribuído', 'supportcandy' );
			$extra_info = esc_attr__( 'Por favor, selecione agente', 'supportcandy' );
			$wpdb->insert(
				$wpdb->prefix . 'psmsc_custom_fields',
				array(
					'name'       => $name,
			...
			$name = esc_attr__( 'Agente criado', 'supportcandy' );
			$wpdb->insert(
				$wpdb->prefix . 'psmsc_custom_fields',
				array(
					'name'       => $name,
			...
			$name = esc_attr__( 'Esperando resposta', 'supportcandy' );
			$wpdb->insert(
				$wpdb->prefix . 'psmsc_statuses',
				array(
					'name'       => $name,
			...
			$labels = array(
				'all'        => esc_attr__( 'Todos', 'supportcandy' ),
				'unresolved' => esc_attr__( 'Não resolvido', 'supportcandy' ),
				'unassigned' => esc_attr__( 'Não atribuído', 'supportcandy' ),
				'mine'       => esc_attr__( 'meus', 'supportcandy' ),
				'closed'     => esc_attr__( 'Fechados', 'supportcandy' ),
				'deleted'    => esc_attr__( 'Apagados', 'supportcandy' ),
			);
			...
			$translations = array(
				1 => array(
					'subject' => 'Seu ticket foi criado com sucesso!',
					'body'    => '<p>Obrigado por entrar em contato. Recebemos sua solicitação!</p>',
				),
				2 => array(
					'body' => '<p>Você recebeu uma nova ocorrência!</p><p><strong>{{customer}}</strong><em>reportado</em></p><p>{{description}}</p><p>{{ticket_url}}</p>',
				),
				3 => array(
					'body' => '<p><strong>{{last_reply_user_name}}</strong> <em>respondeu</em></p><p>{{last_reply}}</p><p>{{ticket_url}}</p><p>{{ticket_history}}</p>',
				),
				4 => array(
					'subject' => 'Sua ocorrência foi fechada!',
					'body'    => '<p>{{customer}},</p><p>Sua ocorrência #{{id}} foi fechada.</p>',
				),
			);
			...
			$subject = 'Registro de usuário OTP - {{otp}}';
			$body = '<p>Hello {{firstname}},</p><p>Abaixo está sua senha única para registro de usuário:</p><p><strong>{{otp}}</strong></p>';
			update_option(
				'wpsc-en-user-reg',
				array(
					'subject' => $subject,
			...
			// misc settings.
			$translation = '<p>Entendo que minhas informações pessoais como nome, endereço de e-mail, endereço IP, etc. serão armazenadas no banco de dados.</p>';
			...
			$translation = '<p>Eu concordo com os Termos e Condições</p>';
			...
			// ticket widgets.
			$labels = array(
				'change-status'         => esc_attr__( 'Status da ocorrência', 'supportcandy' ),
				'raised-by'             => esc_attr__( 'Usuário', 'supportcandy' ),
				'ticket-info'           => esc_attr__( 'Informação da ocorrência', 'supportcandy' ),
				'assignee'              => esc_attr__( 'Assinatura', 'supportcandy' ),
				'ticket-fields'         => esc_attr__( 'Campos da ocorrência', 'supportcandy' ),
			...
			// Dashboard cards.
			$labels = array(
				'new-tickets' => esc_attr__( 'Novas ocorrências', 'supportcandy' ),
				'unresolved'  => esc_attr__( 'Não resolvidas', 'supportcandy' ),
				'unassigned'  => esc_attr__( 'Não atribuidas', 'supportcandy' ),
				'closed'      => esc_attr__( 'Fechadas', 'supportcandy' ),
				'mine'        => esc_attr__( 'Minhas', 'supportcandy' ),
			);
			foreach ( $labels as $key => $string ) {
			...
			// Dashboard widget.
			$labels = array(
				'ticket-statistics'     => esc_attr__( 'Estastiticas', 'supportcandy' ),
				'todays-trends'         => esc_attr__( 'Ocorrências atuais', 'supportcandy' ),
				'agent-list'            => esc_attr__( 'Carga de trabalho', 'supportcandy' ),
				'recent-activities'     => esc_attr__( 'Últimas atividades', 'supportcandy' ),
				'recent-tickets'        => esc_attr__( 'Ocorrências recentes', 'supportcandy' ),
				'category-report'       => esc_attr__( 'Ocorrências não resolvidas por categoria', 'supportcandy' ),
				'unresolved-priorities' => esc_attr__( 'Ocorrências não resolvidas por prioridade', 'supportcandy' ),
				'unresolved-statuses'   => esc_attr__( 'Ocorrências não resolvidas por status', 'supportcandy' ),
			);
			foreach ( $labels as $key => $string ) {
		...
		public static function upgrade() {
			...
			if ( version_compare( self::$current_version, '3.2.7', '<' ) ) {
				...
				$gdpr_text = '<p>Entendo que minhas informações pessoais como nome, endereço de e-mail, endereço IP, etc. serão armazenadas no banco de dados.</p>';
				...
				$tandc_text = '<p>Eu concordo com os Termos e Condições</p>';
